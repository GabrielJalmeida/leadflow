...
def _service_need(
    ...
    if lead.website_status == WebsiteStatus.UNREACHABLE:
        reasons.append("site identificado, mas a última checagem ficou inconclusiva +18")
        cautions.append(
            "falha de DNS/rede/timeout em uma auditoria não prova que o site do cliente está quebrado"
        )
        return (
            OpportunityType.REVIEW_NEEDED,
            "website_reachability_review",
            18,
            reasons,
            cautions,
        )
    ...
    if audit.blocked or not audit.reachable or audit.status_code is None:
        reasons.append("auditoria do site ficou inconclusiva +18")
        cautions.append(
            "não classificar como rebuild até uma checagem HTTP válida confirmar um problema real"
        )
        if audit.error:
