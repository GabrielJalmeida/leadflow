...
def validate_search_request(request: SearchRequest, settings: Settings | None = None) -> SearchRequest:
    ...
    if features.investigate and (request.no_ai or not settings.gemini_api_key):
        raise ValueError("Investigator requer Gemini configurado")
    if features.visual_audit and (request.no_ai or not settings.gemini_api_key):
        raise ValueError("Visual IA requer Gemini configurado")
    if features.browser_audit and importlib.util.find_spec("playwright") is None:
...
def execute_search(
    ...
    *,
    ...
    report = agent.research(
        ...
        visual_audit=features.visual_audit,
