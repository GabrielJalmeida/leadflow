        ...
        *,
        ...
        lead.evidence.append(
            Evidence(
                ...
                confidence=0.90 if audit.reachable else 0.70,
            )
        )
...
def validate_public_http_url(
    ...
    *,
    ...
    try:
        ...
        raise UnsafeWebsiteUrl(f"hostname resolution failed: {exc}") from exc
    if not resolved:
        raise UnsafeWebsiteUrl("hostname did not resolve")
    ...
    for item in resolved:
        try:
            ...
            continue
    if not addresses:
        raise UnsafeWebsiteUrl("hostname did not resolve to an IP address")
    for address in addresses:
        try:
            ip = ipaddress.ip_address(address)
        except ValueError as exc:
            raise UnsafeWebsiteUrl("hostname resolved to an invalid IP") from exc
        if not ip.is_global:
