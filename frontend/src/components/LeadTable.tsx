...
type Props = {
  ...
  onSelectionChange: (lead: LeadCard, checked: boolean) => void
  onToggleAll: (checked: boolean) => void
  onSelect: (lead: LeadCard) => void
}

export function LeadTable({ leads, selected, selectedKeys, onSelectionChange, onToggleAll, onSelect }: Props) {
  ...
  const selectableLeads = leads.filter((lead) => Boolean(lead.lead_key))
  const allSelected = selectableLeads.length > 0 && selectableLeads.every((lead) => selectedKeys.has(lead.lead_key!))
              ...
              >
                <td className="selection-column" onClick={(event) => event.stopPropagation()}>
                  <input
                    ...
                    checked={Boolean(row.original.lead_key && selectedKeys.has(row.original.lead_key))}
                    onChange={(event) => onSelectionChange(row.original, event.target.checked)}
                  />
                </td>
