...
class TestRawDiscoveryAPIModel(unittest.TestCase):
    def test_raw_discovery_round_trips_to_domain(self):
        from leadflow_agent.api import SearchRunCreateModel
        payload = SearchRunCreateModel(segment="marcenaria", city="Praia Grande", raw_discovery=True)
        self.assertTrue(payload.to_domain().raw_discovery)
