...
class WebsiteAuditorTests(unittest.TestCase):
    ...
    def test_dns_resolution_failure_is_operational_not_security_block(self):
        with self.assertRaises(WebsiteResolutionError):
            validate_public_http_url("https://example.com", resolver=failing_resolver)

        result = WebsiteFetcher(resolver=failing_resolver).fetch("https://example.com")
        self.assertFalse(result.blocked)
        self.assertIsNone(result.status_code)
        self.assertIn("hostname resolution failed", result.error or "")

        lead = Lead(name="Empresa X", website="https://example.com")
        outcome = WebsiteAuditor(FakeFetcher(result)).audit(lead)
        self.assertEqual(lead.website_status, WebsiteStatus.UNREACHABLE)
        self.assertFalse(outcome.audit.blocked)
        evidence = lead.evidence[-1]
        self.assertLessEqual(evidence.confidence, 0.25)
