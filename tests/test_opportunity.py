...
class OpportunityIntelligenceTests(unittest.TestCase):
    ...
    def test_unverified_unreachable_site_requires_review_not_rebuild(self):
        lead = Lead(
            name="Takal-like",
            phone="11 98566-5011",
        ...
        self.assertEqual(lead.opportunity.service_fit, "website_reachability_review")
        self.assertFalse(lead.opportunity.actionable)
        self.assertLessEqual(lead.score, 55)
        self.assertTrue(any("não prova" in c for c in lead.opportunity.cautions))
        self.assertTrue(any("verificar identidade" in c for c in lead.opportunity.cautions))

    def test_verified_unreachable_site_still_requires_review(self):
        lead = Lead(
            name="Beta",
            phone="13 99999-2222",
    ...
    def test_blocked_or_inconclusive_audit_does_not_become_rebuild(self):
        lead = Lead(
            name="Gamma blocked",
            phone="13 99999-3333",
            ...
            website_audit=WebsiteAudit(
                ...
                status_code=None,
                technical_score=15,
                error="local/private host blocked",
            ),
        )
        score_lead(lead)
        self.assertEqual(lead.opportunity.type, OpportunityType.REVIEW_NEEDED)
